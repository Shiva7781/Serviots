...
- **Frontend:** [Vercel](https://vercel.com) free tier. `frontend/vercel.json`
  rewrites `/api/*` to the Render backend and everything else to
  `index.html` (so client-side routes like `/dashboard` don't 404 on direct
  load/refresh). `VITE_API_URL=/api` (a relative path, set as a Vercel
  project env var) — the browser only ever talks to its own origin; Vercel's
  edge proxies `/api/*` to Render server-side, invisibly to the browser.

### Why the API is proxied instead of called directly (and a bug this fixed)

The frontend and backend are on unrelated domains (`*.vercel.app` /
`*.onrender.com`). The first deployment had the frontend call the Render URL
directly, which made the refresh-token cookie a **third-party cookie** from
the browser's point of view — set by `onrender.com` while the top-level site
is `vercel.app`. That "worked" in a normal window (browsers still allow
`SameSite=None; Secure` cookies in third-party contexts by default there) but
**silently failed in Incognito**: Incognito blocks third-party cookies
unconditionally, so the cookie was never stored at all. Login still
"succeeded" (the access token comes back in the JSON body, not the cookie),
but reloading the page had nothing to send to `/api/auth/refresh`, so the
silent-refresh-on-load in `AuthContext` correctly treated that as a dead
session and booted the user back to logged-out.

Confirmed by driving a real Incognito Chrome instance (via CDP, not just a
fresh Playwright context — those aren't equivalent to Incognito's cookie
policy) against the live site: `context.cookies()` came back `[]`
immediately after a 200 login response.

The fix removes the cross-site hop entirely: the frontend calls its own
`/api/*`, Vercel's edge relays that to Render behind the scenes, and the
`Set-Cookie` response reaches the browser looking like it came from
`frontend-six-phi-79.vercel.app` — genuinely same-site, so plain
`SameSite=Lax` is enough and there's nothing for Incognito's third-party
cookie policy to block. Local dev is unaffected (frontend/backend already
share `localhost` as their site, just different ports, which is same-site
by the SameSite spec's definition).

To redeploy after local changes: push to `main` (Render auto-deploys on
push), and re-run `vercel --prod` from `frontend/` for the frontend.
